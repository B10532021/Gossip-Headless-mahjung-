...
void Rev2_Player::dataSet()
{
	...
	for (int i = 0; i < 3; i = i + 1)
	{
		for (int j = 0; j < 9; j = j + 1)
		{
			...
			sea.push_back(GameManager::gameManager->Discards.mjcolor[i][j]);
			deck[i * 9 + j] = deck[i * 9 + j] - GameManager::gameManager->Discards.mjcolor[i][j] - Hand.mjcolor[i][j];
	...
	for (int i = 0; i < 4; i++)
	{
		int amount = (GameManager::gameManager->Discards.windsCards >> (i * 3)) & 7;
	...
	for (int i = 0; i < 3; i++)
	{
		int amount = (GameManager::gameManager->Discards.cfbCards >> (i * 3)) & 7;
		sea.push_back(amount);
...
{
	return Throw(MJCard(-1, -1));
}

MJCard Rev2_Player::Throw(const MJCard & card)
{
	dataSet();
	switch (card.color)
	{
